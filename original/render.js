...
function render_hsl(prefix, to_hsl, from_hsl, result) {
...

  {
    ...
    let lowres_data = new Float32Array(
      ...
      for (let j = 0; j < lowres_picker_size; j++) {
        ...
        if (2 * i + 1 < picker_size) {
          let alpha =
            0.25 *
            picker_size *
            (1 - ((hsl_a + 2 * picker_size_inv) ** 2 + hsl_b ** 2));
          alpha = alpha > 1 ? 1 : alpha;
          alpha = alpha < 0 ? 0 : alpha;
        ...
        if (2 * j + 1 < picker_size) {
          let alpha =
            0.25 *
            picker_size *
            (1 - (hsl_a ** 2 + (hsl_b - 2 * picker_size_inv) ** 2));
          alpha = alpha > 1 ? 1 : alpha;
          alpha = alpha < 0 ? 0 : alpha;
        ...
        if (2 * i + 1 < picker_size && 2 * j + 1 < picker_size) {
          let alpha =
            ...
            picker_size *
            (1 -
              ((hsl_a + 2 * picker_size_inv) ** 2 +
                (hsl_b - 2 * picker_size_inv) ** 2));
          alpha = alpha > 1 ? 1 : alpha;
          alpha = alpha < 0 ? 0 : alpha;
...
function render(r, g, b) {
...

  {
    ...
    for (let i = 0; i < lowres_picker_size; i++) {
      ...
      let aa_scale_1 =
        oklab_C_scale * picker_size_inv + Math.abs(max_Ca - max_Cb);
      let aa_scale_2 =
        oklab_C_scale * picker_size_inv + Math.abs(max_Cb - max_Cc);
